...
class LLMClient:
    ...
    def _describe_with_anthropic(self, image_path: Path, prompt: str = None) -> str:
        ...
        try:
            response = requests.post(
                f"{self.base_url}/messages", headers=headers, json=payload, timeout=30
            )
            response.raise_for_status()
    ...
    def chat(self, messages: list) -> str:
        ...
        if self.provider == "openai" or self.provider == "siliconflow":
            ...
            return self._chat_with_anthropic(messages)
        else:
            logger.error(f"不支持的 LLM 提供商: {self.provider}")
            return None

    def _chat_with_openai(self, messages: list) -> str:
        """使用 OpenAI API 对话"""
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {self.api_key}",
