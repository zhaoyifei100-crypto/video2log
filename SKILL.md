...
# LLM 根据画面分析选择检测器和参数
# 示例：检测到电视画面 -> 选择 black_screen 检测器
config = {
    'detector': 'black_screen',  # 检测器名称
    'params': {                   # LLM 配置的参数
        'threshold': 0.7          # 特征丢失 70% 视为黑屏
    },
    'interval': 5,
               # 检查间隔秒
...
- **原理**：检测画面特征密度（Laplacian 梯度）和对比度变化。对摄像头自动增益 (AGC) 有极强抗性。
- **场景**：电视关闭、显示器断电、信号丢失、摄像头被完全遮挡。
- **参数**：
  - `threshold`: 特征丢失比例 (0.0-1.0, 默认0.7)
    - 说明：画面特征（细节丰富度）比基准下降超过此比例视为黑屏
    - 示例：0.7 表示丢失了 70% 的画面细节
  - `region`: 检测区域 [x1, y1, x2, y2] (可选)
    - **使用归一化坐标** (0.0-1.0)：自动适配不同分辨率
      - 示例：`[0.3, 0.2, 0.7, 0.8]` 表示画面中间区域
- **输出**：`var_drop_ratio`, `current_var`, `is_flat`

#### motion (运动检测)
- **能力**：检测画面运动、物体移动、有人出现或任何显著变化
- **原理**：计算前后两帧之间发生变化的像素占总像素的比例
- **场景**：有人进入监控区域、物体被移动、屏幕内容跳转
- **参数**：
...
2. **VLLM 用于二次确认**，过滤 CV 算法的误报。**在生成的 `vllm_prompt` 中必须加入明确的判别准则和图片角色标注**：
   - **明确图片角色**：在发送给 VLLM 的消息中，为每一张图增加明确的文本标签：`[Image 1: Baseline/Normal State]` 和 `[Image 2: Current Suspicious State]`。
   - **强化指令引导**：加入具体的判别准则。
     - *示例*：“请重点对比：基准图中有一个人坐在位置上。如果当前图中那个人消失了，或者只剩下空椅子，请务必返回 is_confirmed_anomaly: true。”
